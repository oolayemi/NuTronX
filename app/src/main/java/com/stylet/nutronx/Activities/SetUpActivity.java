...
public class SetUpActivity extends AppCompatActivity {
                                ...
                                .addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            Toast.makeText(SetUpActivity.this, "User settings updated successfully", Toast.LENGTH_SHORT).show();
                                            Intent toMain = new Intent(SetUpActivity.this, MainActivity.class);
                                            startActivity(toMain);
                                            finish();
                                        } else {
                                            Toast.makeText(SetUpActivity.this, "User settings not updated: " + task.getException().getMessage(), Toast.LENGTH_SHORT)
                                                    .show();
                                        }
                                    }
                                });
    ...
    private void storeFirestore(String user_id, String username, String usermail, String userimageuri){
        ...
        user_details.put("usermail", usermail);
        user_details.put("userimageuri", userimageuri);


        firebaseFirestore.collection("Users").document(user_id).set(user_details).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()){
